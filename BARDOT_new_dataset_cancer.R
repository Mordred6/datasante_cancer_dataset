...
df_wide$Cluster <- as.factor(kmeans_model$cluster)

# Visualisation PCA
pca_model <- prcomp(df_scaled)
fviz_pca_ind(pca_model,
             geom.ind = "point",
             col.ind = df_wide$Cluster,
             palette = "jco",
             addEllipses = TRUE,
...
print(table(df_wide$Cluster))

# Afficher les pays dans chaque cluster
for (i in unique(df_wide$Cluster)) {
  cat("\n Pays dans le Cluster", i, ":\n")
  print(rownames(df_wide[df_wide$Cluster == i, ]))
}
